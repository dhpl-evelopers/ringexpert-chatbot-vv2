...
if "logged_in" not in st.session_state:
    st.session_state.update({
        ...
        "uploaded_file": None,
        "explore_ring_mode": False  # ✅ add it here
    })
...
class Config:
    ...
    try:
        ...
        st.success("✅ Azure secrets loaded successfully")
    except Exception as e:
        st.error(f"❌ Azure secret error: {str(e)}")
...
def show_auth_ui():
    ...
    st.markdown("""
        <div class="welcome-header">
            ...
            <div class="welcome-subtitle">The RingExpert is here to help</div>
        </div>
    """, unsafe_allow_html=True)
...
def show_chat_ui():
    ...
    """, unsafe_allow_html=True)
    if not st.session_state.explore_ring_mode:
    # Existing chat UI
...
if st.session_state.get("explore_ring_mode", False):
    ...
    default_prompt = "Analyze the uploaded ring image. Provide details on diamond size, count, gold quantity, and karat."
    user_prompt = st.chat_input("Enter any specific instruction", key="ring_details_prompt", value=default_prompt)

    if user_prompt:
        handle_user_prompt(user_prompt, uploaded_file)
        st.session_state.explore_ring_mode = False  # Reset after submission

# 📂 Default file upload + input for normal use
st.markdown('<div class="file-upload-container">', unsafe_allow_html=True)

# Default chat prompt (for normal mode)
